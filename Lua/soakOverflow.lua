...
while true do
    ...
    local bestIndex = nil
     local bestMove = nil
commands[p.agentId] = commands[p.agentId] or {}
    ...
    if bestTile then
        commands[p.agentId] = {
            move = { x = bestTile.x, y = bestTile.y}
        }
    ...
    for _,p in ipairs(oppPlayers) do
       if hasNoCover(p.x,p.y) then
        ...
        break
        --no need if pToShoot
        elseif not pToShoot and hasLowCover(p.x,p.y) then
            pToShoot =p
        end

        if pToShoot then
            --not good id should be the shooter agent!! 
            commands[p.agentId] = {
                shoot = {x = pToShoot.x, y = pToShoot.y}
            }
            end
    end
