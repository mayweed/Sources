...
func main() {
	...
	for i := 0; i < C; i++ {
		//Should put that in a recursive func
		//First init the num of riders and queue things accordingly
		var participants = ppl[0]
		ppl = append(ppl[1:], ppl[0])

		if participants < L {
			if ppl[0]+participants <= L {
				participants += ppl[0]
				ppl = append(ppl[1:], ppl[0])
			}
		}
