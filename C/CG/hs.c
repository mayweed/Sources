...
typedef enum { CMD_MOVE, CMD_BOMB } ctype;
...
char get_board(char *board, int w, int x, int y) { return board[y * w + x]; }
...
bool danger_here(char *board, int w, int h, int x, int y, int bomb_x,
                 int bomb_y, int range) {
  if (x == bomb_x && abs(y - bomb_y) <= range)
    return true;
  if (y == bomb_y && abs(x - bomb_x) <= range)
    return true;
  return false;
}
